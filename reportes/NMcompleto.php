...
<html>
	<head>
		<title>
			NM Reporte de Ingresos contra Egresos
		</title>
...
//inicia reporteo
			...
			$egreso = mysqli_query($con, "SELECT * FROM egresosnm")
					or die(mysqli_error($con));

			$ingTotal = 0;
			while($reg = mysqli_fetch_array($ingreso))
			{	$ingTotal = $reg['cap1000'] + $ingTotal;	}

			$egrTotal = 0;
			while($reg = mysqli_fetch_array($egreso))
			{	$egrTotal = $reg['cap1000'] + $egrTotal;	}
			...
			echo '<fieldset id="base">
					...
					<legend>Disponible en N&oacute;mina</legend>';
