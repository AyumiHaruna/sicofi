...
<?php
	...
	$con = mysqli_connect($server, $user, $pass, $database.$_SESSION['anio'])
		...
		if($userProy[0] == 999999){
 			$textoBusqueda .= "ing.tipo = 'REINTEGRO'	 AND ing.validado > 0 ";
		} else {
			for($x=0; $x<count($userProy); $x++){
				if($x<(count($userProy) - 1)){
					$textoBusqueda .= "pro.numeroProyecto = '$userProy[$x]' AND ing.tipo = 'REINTEGRO' OR ";
				} else {
					$textoBusqueda .= "pro.numeroProyecto = '$userProy[$x]' AND ing.tipo = 'REINTEGRO' ";
				}
			}
		}
	...
	<body>
		...
		<fieldset id="base">
			<fieldset id="formulario">
				<legend>Búsqueda de Reintegros</legend>
				<form name="form1">
	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			function pintaIngresos(pro, enc){
				...
				var thisLista = ''+
				'<legend>Lista de Reintegros</legend>'+
				'<table class="tablaReport" border="1">'+
					'<tr>'+
						'<td>No</td>'+
						'<td># Proyecto</td>'+ '<td>Nombre del Proyecto</td>'+
						'<td>cap. 1000</td>'+
						'<td>cap. 2000</td>'+ '<td>cap. 3000</td>'+ '<td>cap. 4000</td>'+
						'<td>cap. 5000</td>'+ '<td>Total</td>'+
					'<tr>';
