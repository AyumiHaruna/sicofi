...
<html>
	<head>
		<title>Registro de Egresos N&oacute;mina</title>
...
<!-- IIIIIIIIIIIIIIIIIIIIIIIIIIII 	Aqui se inicia el formulario  IIIIIIIIIIIIIIIIIIIIIIIIIIII -->
		...
		<fieldset id="base">
			<fieldset id="formulario">
				<legend>Registro de Egresos Nomina</legend>
				<form method="post" action="NMaltaEgresosSube.php" name="form1">
			...
			<fieldset id="formulario">
				...
				<table class="tablaForm">
					<tr>
						<td colspan="2">Importe Total (Cap 1000):     $</td>
						<td colspan="2"><input type="text" name="cap1000" onkeypress="return justNumbers(event);"  required></td>
					</tr>
					<tr>	<td> &nbsp; </td>		</tr>
		...
		//	---------------------  Aquí haremos las búsquedas correspondientes para el calculo de la cantidad Disponible ---------------
			...
			$ingresosnm = mysqli_query($con, "SELECT cap1000 FROM ingresosnm")
				or die(mysqli_error($con));

			$egresosnm = mysqli_query($con, "SELECT cap1000  FROM egresosnm")
				or die(mysqli_query($con));
			//	---sumaremos lo que hay en ingresos---
			$ingTotal = 0;
			while($ing = mysqli_fetch_array($ingresosnm))
			{
				$ingTotal  = $ing['cap1000']  + $ingTotal;
			}
			//	---sumaermos lo que hay en egresos---
			$egrTotal = 0;
			while($egr = mysqli_fetch_array($egresosnm))
			{
				$egrTotal = $egr['cap1000']  + $egrTotal;
			}
		...
		echo	'<tr>
						<td colspan="8"> Actualmente hay Disponible: <font color="blue">$'.number_format($total,2,'.',',').'</font></td>
					</tr>';
		?>
				</table>
			</fieldset>
