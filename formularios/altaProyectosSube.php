<?php
	...
	//-----------------------------------------------------------
	if($_POST['tipo'] == 'creaProy')
	{
		$numeroProy = $_POST['numeroProyecto'];
		$nombreProyecto = $_POST['nombreProyecto'];
		$nombreResponsable = $_POST['nombreResponsable'];
		$cuenta = $_POST['cuenta'];
		$tipo = (ISSET($_POST['tipoCuenta']))? $_POST['tipoCuenta'] : 1 ;
		if($_POST['titulo'] == '-'){
			$titulo = "";
		} else {
			$titulo = $_POST['titulo'];
		...
		if( $_SESSION['anio'] <= 2018 ){
			mysqli_query($con, "insert into proyectos(numeroProyecto, nombreProyecto, nombreresponsable, cuenta, titulo)
									values('$numeroProy','$nombreProyecto','$nombreResponsable', '$cuenta', '$titulo')")
				or die(mysqli_error($con));
		} else {
			mysqli_query($con, "insert into proyectos(numeroProyecto, nombreProyecto, nombreresponsable, cuenta, titulo, tipo)
									values('$numeroProy','$nombreProyecto','$nombreResponsable', '$cuenta', '$titulo', '$tipo')")
				or die(mysqli_error($con));
		}

		echo 'ok';
	}    //--	termina crea proyecto
	...
	if($_POST['tipo'] == 'createPartidas')
	{
		...
		//revisa el tipo de proyecto, si es Gasto basico, 1 o Nominas 2, genera los ingresos
