...
//CONSULTA A LA BASE DE DATOS
	...
	$egresos = mysqli_query($con, "SELECT noCheque, fechaElaboracion, nombre, concepto, cap1000
										FROM egresosnm
	...
	while ($reg = mysqli_fetch_array ($egresos))
		{
			...
			$cap10001 = number_format($reg['cap1000'], 2,'.',',');
		//	$pdf -> Cell(0,1, number_format($reg['cap1000'], 2,'.',','), 0,1, 'R');
			$pdf -> Cell(0, 1, "$ $cap10001 ", 0, 1, 'L');

			//Monto en letra
			$pdf -> Ln(0);
			$pdf -> SetLeftMargin(5);
			$pdf -> SetFont('Arial', '', 8);
			$pdf -> Cell(0, 1,' ('.MontoMonetarioEnLetras($reg['cap1000']).')', 0, 1, 'L');
