<?php
	...
	require_once("../fpdf/fpdf.php");	//Llamamos la librer�a
		//inicia la elaboraci�n de la poliza con fpdf
	require_once("../fpdf/fpdf.php");	//Llamamos la librer�a
...
$a = 455;
//Almacenamos la variable que pas� del anterior formulario
while($a <= 489)
	{
	...
	//-----------------------------------------------------------
		...
		//Primer l�nea de la pagina comienza por el No. de Solicitud de Fondos
		$pdf -> SetXY(2, 3);
		...
		$pdf -> SetXY(3.8, 7);
		$pdf -> SetFont('Arial', '', 8); //tipo de fuente
		$pdf -> Cell(0, 1, 'DRA. VALERIE MAGAR MEURS', 0, 1, 'L');
