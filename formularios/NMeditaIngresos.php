...
//inicia busqueda de proyectos
		...
		$busqueda = mysqli_query($con, "SELECT *  FROM ingresosnm")
			or die(mysqli_error($con));

		echo '
		<fieldset id="base">
			<fieldset id="formulario">
				<legend>Editor de Ingresos Nomina</legend>
				<form method="post" action="NMeditaIngresos.php" name="form1">
						...
						<tr>
							...
							<td>
								<select name="val1">
									...
									<option value="cap1000">Monto Total</option>
									</select>
							</td>
						</tr>
		...
		while($reg = mysqli_fetch_array ($ingreso))
		{
			echo '
							...
							<tr>
								<td><a href="NMeditaIngresos2.php?codigo='.$reg['no'].'"><img src="../Imagen/writeNegro.png" width="25" height="25"></img></a></td>
								<td>'.$reg['no'].'</td>
								<td>'.$reg['tipo'].'</td>
								<td>'.$reg['concepto'].'</td>
								<td>'.$reg['mes'].'</td>
