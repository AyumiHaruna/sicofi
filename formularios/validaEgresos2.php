...
<!--  /////////////////////////// MAIN DIV ////////////////////////////////// -->
		<div class="container">
				<div class="col-md-12 bloque">
					<form method="post" action="validaEgresosSube.php" name="form1">
		        <div class="col-md-4 subBloque">
		          ...
		          <legend>DATOS DE VALIDACIONES</legend>
							...
							<div class="row">
								<?php
									echo '<div class="col-xs-4"> Comprobaciones $ </div>
												<div class="col-xs-4"> Validaciones </div>
												<div class="col-xs-4"> Fecha de Comprobacion </div>';
									for($x=1; $x<=6; $x++)
									{
										echo '<div class="col-xs-4"> <input type="text" name="comprobacion'.$x.'" id="comprobacion'.$x.'" class="form-control comprobacion input" onkeypress="return justNumbers(event);"> </div>
													<div class="col-xs-4">
														<select name="validacionComp'.$x.'" id="validacionComp'.$x.'" class="form-control input">
															<option value="0" selected> </option>
															<option value="1"> Validado </option>
															<option value="2"> No Validado </option>
														</select>
													</div>
													<div class="col-xs-4"> <input type="date" name="fechaComp'.$x.'" id="fechaComp'.$x.'" class="form-control input"> </div>';
									}
								?>
							</div>
							...
							<div class="row bloqueExtra">
								<!-- SE PINTA EL BOTON SI SON VIATICOS -->
							</div>
						</div>
					</form>
					<br>
		...
		<script type="text/javascript">
	      ...
	      $("#btnRegresar").click(function(){
	        history.back();
				...
				//-imprime el recibo de Gastos no Comprobables-
				//---------------------------------------------
				$(".bloqueExtra").on('click', '#gnc', function(){
					var location = "imprimeGNC.php?codigo=<?php echo $_GET['codigo'] ?>";
				...
				$("#scGuardar").click(function(){
					if( $("#scFolio").val() == '' || $("#scMonto").val() == '' ){
						...
						$.ajax({
								data: { type:'postQr', noCheque:thisNoCheque, cadena:$("#scFolio").val(), tipo:'PR', monto:$("#scMonto").val(), duplicado:0, autoriza:'', observaciones:''},
								type: "POST",
								url: "validaEgresosSube.php",
						})
						.done(function(data){
	      ...
	      //------------------------------------------------------------
				...
				function guardaCodigo()
				{
					...
					if( thisArr[1] != null)	{
						...
						if( monto != NaN ){
							//guardamos en la db
							$.ajax({
			            data: { type:'postQr', noCheque:thisNoCheque, cadena, tipo:'PR', monto, duplicado, autoriza, observaciones},
			            type: "POST",
			            url: "validaEgresosSube.php",
			        })
							.done(function(data){
		            ...
		            muestraAviso('', '<i class="fa fa-exclamation-triangle fa-lg" aria-hidden="true"></i> &nbsp;&nbsp; No se encuentra la Base de Datos, favor de reintentarlo');
								...
								muestraAviso('', '<i class="fa fa-exclamation-triangle fa-lg" aria-hidden="true"></i> &nbsp;&nbsp; Código inválido (monto)');
						}
					} else {
						muestraAviso('', '<i class="fa fa-exclamation-triangle fa-lg" aria-hidden="true"></i> &nbsp;&nbsp; Código inválido (tt)');
					}
				}
	            ...
	            data: { type:'postCambios', noCheque:thisNoCheque,
											comprobacion1 : $("#comprobacion1").val(),	comprobacion2 : $("#comprobacion2").val(),
											comprobacion3 : $("#comprobacion3").val(),	comprobacion4 : $("#comprobacion4").val(),
											comprobacion5 : $("#comprobacion5").val(),	comprobacion6 : $("#comprobacion6").val(),
											validacionComp1 : $("#validacionComp1").val(),	validacionComp2 : $("#validacionComp2").val(),
											validacionComp3 : $("#validacionComp3").val(),	validacionComp4 : $("#validacionComp4").val(),
											validacionComp5 : $("#validacionComp5").val(),	validacionComp6 : $("#validacionComp6").val(),
											fechaComp1 : $("#fechaComp1").val(), fechaComp2 : $("#fechaComp2").val(),
											fechaComp3 : $("#fechaComp3").val(), fechaComp4 : $("#fechaComp4").val(),
											fechaComp5 : $("#fechaComp5").val(), fechaComp6 : $("#fechaComp6").val(),
				...
				function muestraAviso(tipo, mensaje)
        {
          ...
          setTimeout( function(){
            ...
            data = $.parseJSON(data);
						...
						for(var x=1; x<=6; x++)
						{
							$("#comprobacion"+x).val( data['comprobacion'+x] );
							$("#validacionComp"+x).val( data['validacionComp'+x] );
							$("#fechaComp"+x).val( data['fechaComp'+x] );
						}
						...
						console.log(data['viaticos']);
						if( data['viaticos'] == "1"){
							$(".bloqueExtra").html('<div class="col-md-12 text-right"> <button type="button" id="gnc" class="btn btn-lg btn-block submit_button"><i class="fa fa-print" aria-hidden="true"></i> &nbsp; 	IMPRIMIR RECIBO GNC</button>  </div>');
						}
	        })
	        .fail(function(){
	          muestraAviso('', '<i class="fa fa-exclamation-triangle fa-lg" aria-hidden="true"></i> &nbsp;&nbsp; No se encuentra la Base de Datos, favor de reintentarlo');
				...
				function calculaMontos()
				{
					...
					for(var x=1; x<=6; x++)
					{
						if( $("#comprobacion"+x).val() != "") {
							comprobado += parseFloat($("#comprobacion"+x).val());
						}
					}
