	...
	while ($reg = mysqli_fetch_array ($egresos))
		{
			...
			$importeTotal1 = number_format($reg['importeTotal'], 2,'.',',');
		//	$pdf -> Cell(0,1, number_format($reg['importeTotal'], 2,'.',','), 0,1, 'R');
			$pdf -> Cell(0, 1, "$ $importeTotal1 ", 0, 1, 'L');

			//Monto en letra
			$pdf -> Ln(0);
			$pdf -> SetLeftMargin(5);
			$pdf -> SetFont('Arial', '', 8);
			$pdf -> Cell(0, 1,' ('.MontoMonetarioEnLetras($reg['importeTotal']).')', 0, 1, 'L');

			//Numero de cuenta
			$pdf -> Ln(0.5);
			$pdf -> SetX(1.8);
			$pdf -> SetFont('Arial', '', 11);
			$pdf -> Cell(0, 1, "CUENTA: $cuenta " , 0, 1, 'l');

			//Numero de Cheque
			$pdf -> Ln(-1);
			$pdf -> SetX(18.2);
			$pdf -> SetFont('Arial', '', 10);
			$pdf -> Cell(0, 1, $reg['noCheque'], 0, 1, 'l');

			//Concepto del cheque
			$pdf -> Ln(0.1);
			$pdf -> SetX(1.7);
			$pdf -> SetFont('Arial', '', 7);
			$pdf -> MultiCell(8.9, 0.3, $reg['concepto'].'.' , '0');
			...
			$pdf -> MultiCell(4.8, 0.3, 'PROYECTO ' , 0);

			//Numro de Proyecto
			$pdf -> Ln(-0.66);
			$pdf -> SetX(4.8);
			$pdf -> SetFont('Arial', '', 8);
			$pdf -> Cell(0, 1, $reg['numeroProyecto'] , 0, 1, 'l');

			//Nombre del Proyecto
			$pdf -> Ln(-1);
			$pdf -> SetXY(5.8, 10.8);
			$pdf -> SetFont('Arial', '', 7);
			$pdf -> MultiCell(4.8, 0.3, '-  '.$reg['nombreProyecto'].'.' , 0);
			...
			$pdf -> SetX(6.5);
			$pdf -> SetFont('Arial', '', 10);
			$pdf -> Cell(0, 1, "BANCO CTA. $cuenta"  , 0, 1, 'l');
