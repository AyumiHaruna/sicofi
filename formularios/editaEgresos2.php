				...
				<div class="container mainDiv">
					<div class="col-md-12 bloque">
						<div class="col-md-12 subBloque">
							...
							<div class="row">
								...
								<div class="col-xs-3">
									<input type="text" name="noCheque" id="noCheque" class="noCheque form-control" value="<?php echo $_GET['codigo'] ?>" readOnly>
							  </div>
							</div>
							<div class="row">
					...
					<div class="col-md-4 bloque">
						<div class="col-md-12 subBloque">
							...
							Total Autorizado: &nbsp; <span id="disT" class="spanAzul"></span>	<br>
						</div>
					</div>
		...
		<script type="text/javascript">
	      ...
	      //------------------------------------------------------------
            ...
            muestraAviso('', '<i class="fa fa-exclamation-triangle fa-lg" aria-hidden="true"></i> &nbsp;&nbsp; No se encuentra la Base de Datos, favor de reintentarlo');
				...
				//---------------------------------------------
				function getInfoCheque()
				{
					$.ajax({
	            data: { type:'getInfoCheque', noCheque:$("#noCheque").val()},
	            type: "POST",
	            url: "editaEgresosSube.php",
	        })
					.done(function(data){
						data = $.parseJSON(data);
						$("#folio").val(data['folio']); $("#fechaElaboracion").val(data['fechaElaboracion']);
						$("#nombre").val(data['nombre']); $("#concepto").val(data['concepto']);
						$("#proyecto").val(data['nombreProyecto']);	getMontosProyecto();
						if( data['viaticos'] == 1){
							$("#viaticos").prop('checked', true);
							$("#iniVig").val(data['iniVig']); $("#finVig").val(data['finVig']);
							testViaticos();
							validaFechas();
						}
						$("#observaciones").val(data['observaciones']); $("#cap1000").val(data['cap1000']); $("#cap2000").val(data['cap2000']);
						$("#cap3000").val(data['cap3000']); $("#cap4000").val(data['cap4000']); $("#cap5000").val(data['cap5000']);
						$("#total").val(data['totalAutorizado']);
					})
					.fail(function(){
            muestraAviso('', '<i class="fa fa-exclamation-triangle fa-lg" aria-hidden="true"></i> &nbsp;&nbsp; No se encuentra la Base de Datos, favor de reintentarlo');
          });
	            ...
	            url: "editaEgresosSube.php",
					...
					.done(function(data){
						...
						for(var x=1; x<=5; x++)
						{
							var thisVal = (parseFloat(data['cap'+x+'000']).toFixed(2))
							gCap[x] = parseFloat(data['cap'+x+'000']);
							$("#dis"+x).html(' $ '+ addCommas(thisVal) );
						}
						$("#disT").html(' $ '+ addCommas( data['totalAutorizado'] ) )
						gTotalAutorizado = parseFloat(data['totalAutorizado']);
					})
					.fail(function(){
            muestraAviso('', '<i class="fa fa-exclamation-triangle fa-lg" aria-hidden="true"></i> &nbsp;&nbsp; No se encuentra la Base de Datos, favor de reintentarlo');
          });
	            ...
	            url: "editaEgresosSube.php"
					...
					.done(function(data){
						//console.log(data)
						if(data == "ok"){
							muestraAviso('success', '<i class="fa fa-check fa-lg" aria-hidden="true"></i> &nbsp;&nbsp; Se actualizaron los datos exitosamente');
						} else {
							muestraAviso('', '<i class="fa fa-exclamation-triangle fa-lg" aria-hidden="true"></i> &nbsp;&nbsp; No se encuentra la Base de Datos, favor de reintentarlo');
						}
					})
									...
									if( $("#concepto").val() != "") {
										...
										if( $("#proyecto").val() != "") {
											...
											if($("#viaticos").is(":checked")) {
												...
												datos['viaticos'] = 0;
												datos['iniVig'] = '0000/00/00';
												datos['finVig'] = '0000/00/00';
											}
											for(var x=1; x<=5; x++){
