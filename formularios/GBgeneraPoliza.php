...
//CONSULTA A LA BASE DE DATOS
	...
	$egresos = mysqli_query($con, "SELECT noCheque, fechaElaboracion, nombre, concepto, total
										FROM egresosgb
	...
	while ($reg = mysqli_fetch_array ($egresos))
		{
			...
			$total1 = number_format($reg['total'], 2,'.',',');
		//	$pdf -> Cell(0,1, number_format($reg['total'], 2,'.',','), 0,1, 'R');
			$pdf -> Cell(0, 1, "$ $total1 ", 0, 1, 'L');
			...
			$pdf -> Cell(0, 1, 'CUENTA: 70022370-748', 0, 1, 'l');
			...
			$pdf -> MultiCell(4.8, 0.3, utf8_decode('GASTO BÁSICO') , 0);
			...
			$pdf -> SetX(6.5);
			$pdf -> SetFont('Arial', '', 10);
			$pdf -> Cell(0, 1, 'BANCO CTA. 70022370-748' , 0, 1, 'l');
