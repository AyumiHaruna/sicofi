<?php
	...
	$total 		= $_GET['c'];	$noCheque 			= $_GET['d'];
	$total 		= $_GET['c'];	$noCheque 			= $_GET['d'];
	$concepto			= $_GET['e'];	$proyecto 			='Gasto Básico';
/*
...
//-----------------------------------------------------------
	...
	//$pdf -> Cell(0, 1, "$ $total", 0, 1, 'L');
	$pdf -> Cell(0,1, '$'.number_format($total, 2,'.',','), 0,1, 'R');

	//Monto en letra
	$pdf -> Ln(0);
	$pdf -> SetLeftMargin(5);
	$pdf -> SetFont('Arial', '', 8);
	$pdf -> Cell(0, 1,' ('.MontoMonetarioEnLetras($total).')', 0, 1, 'L');
